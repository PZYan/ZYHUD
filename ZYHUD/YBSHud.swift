...
public class YBSHud: UIView {
    ...
    open var config: YBSChrysanConfig = .default()
    ...
    internal class func ybsHud(withView parent: UIView) -> YBSHud? {
        
        if let views = bundle.loadNibNamed("YBSHud", owner: nil, options: nil) as? [YBSHud], views.count > 0 {
            let ybsHud = views[0]
            ybsHud.setup(withView: parent)
            return ybsHud
        }
        return nil
    }
    ...
    private class var bundle: Bundle {
        ...
        let framework = Bundle(for: YBSHud.classForCoder())
        if let resource = framework.path(forResource: "YBSHud", ofType: "bundle") {
    ...
    private func updateAndShow() {
        ...
        positionX.constant = CGFloat(config.offsetX)
    
        positionY.constant = CGFloat(config.offsetY)
    ...
    private func image(name: String) -> UIImage? {
        return UIImage(named: "chrysan_\(name).png", in: YBSHud.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
    }
